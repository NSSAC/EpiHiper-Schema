...
class EpiHiperAddNoise {
   ...
   * @returns {object} EpiHiperAddNoise
   */
  constructor(path, distribution, sigma, min, max, value, replace, integer, negative, validate) {
    if (!(this instanceof EpiHiperAddNoise)) {
      return new EpiHiperAddNoise(path, distribution, sigma, min, max, value, replace, integer, negative, validate);
    }
    ...
    this.validate = (typeof validate === 'undefined') ? false : validate;
    ...
    if (this.distribution === undefined) {
      throw ('EpiHiperAddNoise: Missing distribution [normal|uniform|fixed|raw]');
    }

    if (this.distribution != 'normal' && this.distribution != 'uniform' && this.distribution != 'fixed' && this.distribution != 'raw') {
      throw ('EpiHiperAddNoise: Invalid distribution [normal|uniform|fixed|raw]');
    }
